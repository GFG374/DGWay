...
| 模型白名单 | 包含实际可用模型；Antigravity 分组不要直接暴露默认 22 个模型，只展示实测通过的模型 |
...
GPT 聊天、Codex、图片生成在 DGWay 里应该按不同模型和分组计费。图片生成要单独控制，因为成本更高，不建议默认开放给所有用户。
...
`claude-opus-4-8` 当前不可用，不要配置到 Claude Code 的 Opus 角色里。Claude Code 建议映射到 `claude-opus-4-6` 或 `claude-opus-4-6-thinking`。
