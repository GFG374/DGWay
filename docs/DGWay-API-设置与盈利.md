...
支付页在创建余额充值或订阅套餐订单前，会弹出支付规则确认：恶意支付、争议支付、无效转账、重复支付，或未按页面订单金额完成的支付，平台不承诺退款；用户确认后才会真正创建支付订单。
...
| 邮箱注册 | 注册开关开启时，登录页会显示“注册”入口，用户可用邮箱和密码创建账号 |
| 文档地址 | 指向使用说明页或飞书文档 |
| API Base URL | `https://dgth.shop/v1` |
...
账号购买页用于展示站长自营商品，例如 OpenAI 成品账号、Gemini 成品账号、长效手机号接码、美国静态住宅 IP 等。这里是展示和联系方式入口，不负责自动发货。

可配置内容：

| 配置项 | 用途 |
| --- | --- |
| 联系方式 | 修改 QQ、微信、邮箱或其他联系信息 |
| 商品名称 | 例如 OpenAI 成品账号、Gemini 成品账号 |
| 价格和单位 | 例如 30 元/个、40 元/30 天 |
| 标签和图标 | 控制卡片右上角标签；商品图标可上传 PNG、JPG、WEBP 或 GIF，系统会压缩成统一尺寸 |
| 功能点 | 用中文逗号或英文逗号分隔，前台逐行展示 |
| 风险提示 | 用于说明风控、库存、绑定数量等不确定性 |
| 是否启用 | 暂时下架某个商品时关闭即可 |

桌面端默认每行 3 个商品，超过 3 个会自动换行。后续新增 Gemini、Claude、代开账号或其他服务时，直接新增商品并上传对应图标即可，不需要重新改前端。
