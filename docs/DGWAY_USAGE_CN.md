...
管理员修改入口：`系统设置 -> 通用设置 -> 账号购买设置`。可以修改页面标题、说明、联系方式、底部提示，也可以新增、复制、删除、上移或下移商品。商品图标支持直接上传 PNG、JPG、WEBP 或 GIF，系统会压缩成统一尺寸，前台按当前图标框显示。商品“功能点”输入框用中文逗号 `，` 或英文逗号 `,` 分隔，保存后前台会自动逐行展示；桌面端默认每行 3 个商品，超过 3 个自动换到下一行。
